...
if (!isset($_SESSION["account_branch"]) || empty($_SESSION["account_branch"])) {
    die("No Account Branch session found.");
}

$ad = $_SESSION["account_branch"];

// Fetch user data
$query = "SELECT * FROM account_branch WHERE username = ?";
...
if (isset($_POST["update"])) {
    if (isset($_FILES["profile"]) && $_FILES["profile"]["error"] == UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($profileTmp, $uploadPath)) {
            $updateQuery = "UPDATE account_branch SET profile = ? WHERE username = ?";
...
if (isset($_POST["change"])) {
    $uname = $_POST["uname"];
    if (!empty($uname)) {
        $query = "UPDATE account_branch SET username = ? WHERE username = ?";
        $stmt = mysqli_prepare($connect, $query);
...
if (isset($_POST["update_pass"])) {
    ...
    $query = "SELECT password FROM account_branch WHERE username = ?";
    $stmt = mysqli_prepare($connect, $query);
    mysqli_stmt_bind_param($stmt, "s", $ad);
    mysqli_stmt_execute($stmt);
    $res = mysqli_stmt_get_result($stmt);
    ...
    if (empty($old_pass)) {
        ...
        $query = "UPDATE account_branch SET password = ? WHERE username = ?";
        $stmt = mysqli_prepare($connect, $query);
...
<body>
    ...
    <div class="sidenav" style="margin-top: -787px; margin-left: -2px;">
