...
if (isset($_POST["update"])) {
    if (isset($_FILES["profile"]) && $_FILES["profile"]["error"] == UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($profileTmp, $uploadPath)) {
            $updateQuery = "UPDATE doctors SET profile = ? WHERE username = ?";
...
if (isset($_POST["update_pass"])) {
    ...
    $query = "SELECT password FROM doctors WHERE username = ?";
    $stmt = mysqli_prepare($connect, $query);
    mysqli_stmt_bind_param($stmt, "s", $ad);
    mysqli_stmt_execute($stmt);
    $res = mysqli_stmt_get_result($stmt);
...
<body>
    ...
    <div class="sidenav" style="margin-top: -787px; margin-left: -5px;">
