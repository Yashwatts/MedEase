...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['appointment_id'])) {
    ...
    switch ($action) {
        case 'approve':
            $sql = "UPDATE general_appointment SET status = 'Accepted' WHERE id = '$appointment_id'";
            break;
        case 'deny':
            $sql = "UPDATE general_appointment SET status = 'Denied' WHERE id = '$appointment_id'";
            break;
        default:
            echo "Invalid action.";
            exit();
    ...
    if ($conn->query($sql) === TRUE) {
        header("Location: general_appointment.php"); // Redirect to the same page after action
        exit();
    } else {
...
if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['search_phone'])) {
    ...
    $sql = "SELECT id, name, phone, gender, department, doctor, status 
            FROM general_appointment 
            WHERE doctor = '$doctor_name'
            AND (phone = '$search_phone' OR status = 'pending')";
} else {
    $sql = "SELECT id, name, phone, gender, department, doctor, status 
            FROM general_appointment WHERE doctor = '$doctor_name'";
}
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        if ($row['status'] == 'pending') {
                            ...
                            echo "Accepted <span class='checkmark'>✔️</span>";
                        }
